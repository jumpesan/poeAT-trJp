...
import { stat, STAT_BY_REF_V2, pseudoStatByRef,ITEM_BY_TRANSLATED } from '@/assets/data'
...
export function createTradeRequest (filters: ItemFilters, stats: StatFilter[]) {
  ...
  const hasFracturedStatSearch = stats.some(stat =>
    !stat.disabled &&
    (stat.fracturedMode === 'fractured' || stat.fracturedMode === 'both')
  )

  console.log('[APT DEBUG] createTradeRequest START', {
    filters,
  ...
  if (filters.fractured?.value === false && !hasFracturedStatSearch) {
  ...
  for (const stat of stats) {
    console.log('[APT DEBUG] Processing stat', {
      statRef: stat.statRef,
      text: stat.text,
      tradeId: stat.tradeId,
      firstTradeId: stat.tradeId?.[0],
      tags: stat.tags,
      option: stat.option,
  ...
  type BareStatFilter = Omit<StatFilter, 'text' | 'tags' | 'sources' | 'statRef'> & {
    statRef?: StatFilter['statRef']
    sources?: StatFilter['sources']
  }
  ...
  const activeRealStats = realStats.filter(stat => !stat.disabled)

  const groupedActiveStats = new Map<string, BareStatFilter[]>()

  for (const stat of activeRealStats) {
    const ref = stat.statRef ?? stat.sources?.[0]?.stat.stat.ref
    if (!ref) continue

    const group = groupedActiveStats.get(ref)
    if (group) {
      group.push(stat)
    } else {
      groupedActiveStats.set(ref, [stat])
    }
  }

  for (const stat of realStats) {
    const ref = stat.statRef ?? stat.sources?.[0]?.stat.stat.ref
    const sameStats = ref ? groupedActiveStats.get(ref) : undefined

    if (!stat.disabled && sameStats && sameStats.length >= 2 && sameStats[0] !== stat) {
      continue
    }

    if (!stat.disabled && sameStats && sameStats.length >= 2) {
      const merged = mergeSameStatFilters(sameStats)
      qAnd.filters.push(tradeIdToQuery(merged.tradeId[0], merged))
      continue
    }
    if (stat.not) {
      ...
      if (stat.fracturedMode === 'both') {
        const fracturedId = getFracturedTradeId(stat)
        ...
        query.stats.push({
          ...
          disabled: stat.disabled,
          filters: [
            ...stat.tradeId.map(id => tradeIdToQuery(id, { ...stat, fracturedMode: 'explicit' })),
            ...(fracturedId ? [tradeIdToQuery(fracturedId, { ...stat, fracturedMode: 'fractured' })] : [])
          ]
        })
...
export async function requestTradeResultList (body: TradeRequest, leagueId: string): Promise<SearchResult> {
  ...
  if (!data) {
    ...
    //console.log('[TRADE_REQUEST]', JSON.stringify(body, null, 2))
...
function tradeIdToQuery (
  id: string,
  stat: Pick<StatFilter, 'roll' | 'option' | 'disabled' | 'fracturedMode'> & StatWithOptionalSources
) {
  let roll = stat.roll

  if (stat.fracturedMode === 'fractured') {
    id = getFracturedTradeId(stat) ?? id
  }
...
function nameToQuery (name: string, filters: ItemFilters) {
  const translated =
    ...
    ?? ITEM_BY_TRANSLATED('GEM', name)?.[0]

  const queryName = translated?.refName ?? name
...
type StatWithOptionalSources = {
  sources?: StatFilter['sources']
}

function getFracturedTradeId (
  stat: StatWithOptionalSources
): string | undefined {
  return stat.sources
    ?.flatMap(source => source.stat.stat.trade.ids[ModifierType.Fractured] ?? [])
    .find(Boolean)
}

function mergeSameStatFilters<T extends {
  tradeId: string[]
  roll?: StatFilter['roll']
  disabled?: boolean
  ...
  const rolls = stats
    .map(stat => stat.roll)
    .filter((roll): roll is NonNullable<StatFilter['roll']> => roll != null)
  ...
  return {
    ...base,
    tradeId: base.tradeId.map(id => id.replace('fractured.', 'explicit.')),
    roll: rolls.length
      ? {
          ...rolls[0],
          value: sum(rolls.map(r => r.value)) ?? rolls[0].value,
          min: sum(rolls.map(r => r.min)),
          max: sum(rolls.map(r => r.max)),
          default: {
            ...rolls[0].default,
            min: sum(rolls.map(r => r.default.min)) ?? rolls[0].default.min,
            max: sum(rolls.map(r => r.default.max)) ?? rolls[0].default.max
          }
        }
      : undefined,
    disabled: false
