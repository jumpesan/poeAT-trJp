...
export const PROPERTY_USED_STATS = new Set<string>([
  ...ARMOUR_STATS,
  ...WEAPON_STATS,
])
...
function hideUsedStats (ctx: FiltersCreationContext, stats: Set<string>) {
  for (const filter of ctx.filters) {
    if (stats.has(filter.statRef)) {
      filter.hidden = 'filters.hide_property_source'
...
export function propToFilter (opts: {
  ...
  filter.tags = [FilterTag.Property]
