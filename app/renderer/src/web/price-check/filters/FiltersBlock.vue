  ...
  <div>
    ...
    <div v-if="!statsVisibility.disabled && hasStats" class="mb-4" :class="(presets.length > 1) ? 'mt-1' : 'mt-4'">
      ...
      <form @submit.prevent="handleStatsSubmit">
        <filter-modifier v-for="filter of filteredStats" :key="filter.tags.join('+') + '/' + filter.text"
          :filter="filter"
          :item="item"
      ...
      <div class="flex gap-x-4">
        ...
        <ui-toggle v-if="canToggleHidden"
  ...
  setup (props, ctx) {
    ...
    return {
      ...
      filteredStats: computed(() => {
        const hasVisibleStats = props.stats.some(s => !s.hidden)

        if (props.stats.length > 0 && !hasVisibleStats) {
          return props.stats
        }
      ...
      canToggleHidden: computed(() => {
        const hasVisibleStats = props.stats.some(s => !s.hidden)
        const hasHiddenStats = props.stats.some(s => s.hidden)

        return hasVisibleStats && hasHiddenStats
      }),
      showUnknownMods,
      hasStats: computed(() =>
        props.stats.length ||
