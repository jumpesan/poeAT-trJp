    ...
    <div class="flex flex-col min-w-0 flex-1">
      <div class="pb-px flex items-baseline justify-between">
        ...
        <i class="w-5" :class="{
          ...
          'fas fa-check-square text-yellow-400': !isDisabled && filter.mergeGroup != null,
          'fas fa-check-square': !isDisabled && filter.mergeGroup == null
        }"></i>
          <div class="search-text flex-1 mr-1 relative flex min-w-0" style="line-height: 1rem;">
            <span class="truncate"><item-modifier-text :text="displayText" :roll="fractureDisplayRoll?.value" :show-plus="showPlusForRoll" /></span>
            <span class="search-text-full whitespace-pre-wrap"><item-modifier-text :text="displayText" :roll="fractureDisplayRoll?.value" :show-plus="showPlusForRoll" /></span>
          </div>
        </button>
      ...
      <div class="flex">
        ...
        <div class="flex-1 min-w-0 flex items-start gap-x-2">
          ...
          <span v-for="(tag, idx) of displayTags"
            :key="tag"
            :class="[$style['tag'], $style[`tag-${tag}`]]">{{ t(`filters.tag_${tag.replace('-', '_')}`) }}{{ (idx === displayTags.length - 1 && filter.sources.length > 1) ? ` x ${filter.sources.length}` : null }}</span>
          <span
            v-if="filter.mergeGroup != null"
            :class="[$style['tag'], $style['tag-merged']]"
          >
            {{ t('filters.tag_merged') }}{{ mergeGroup }}
          </span>
...
import { FilterTag, StatFilter, INTERNAL_TRADE_IDS, hasFilterTag } from './interfaces'
...
export default defineComponent({
  ...
  setup (props, ctx) {
    const displayTags = computed(() => props.filter.tags)
    ...
    const displayText = computed(() => {
      const value = props.filter.roll?.value
  ...
  const displayRoll = computed(() => {
    const mode = props.filter.fracturedMode ?? 'explicit'

    if (!props.filter.roll) return undefined

    const targets =
      mode === 'fractured'
        ? fracturedSources.value
        : mode === 'both'
          ? [...explicitSources.value, ...fracturedSources.value]
          : explicitSources.value

    if (!targets.length) {
      return props.filter.roll
    }
    ...
    const showPlusForRoll = computed(() => {
      const value = props.filter.roll?.value
      ...
      if (hasFilterTag(props.filter, FilterTag.Property)) {
        return false
      }

      return props.filter.sources.some(source => {
        const statRef = source.stat.stat.ref
        const translation = source.stat.translation.string

        return statRef.includes('+#') &&
    ...
    const explicitSources = computed(() =>
      props.filter.sources.filter(source => !source.fractured)
    )

    const fracturedSources = computed(() =>
      props.filter.sources.filter(source => source.fractured)
    )

    const canFractureSearch = computed(() =>
      fracturedSources.value.length > 0
    )

    const fractureModeLabel = computed(() => {
      switch (props.filter.fracturedMode ?? 'explicit') {
        case 'fractured':
          return 'フラクチャー'
        case 'both':
          return '合算'
    ...
    function toggleFracturedMode () {
      const current = props.filter.fracturedMode ?? 'explicit'

      props.filter.fracturedMode =
        current === 'explicit'
          ? 'fractured'
          : current === 'fractured'
            ? 'both'
    ...
    function sumSourcesValue (sources: typeof props.filter.sources): number | undefined {
    ...
    const fractureDisplayRoll = computed(() => {
      if (!props.filter.roll) return undefined
      ...
      const sources =
        mode === 'fractured'
          ...
          : mode === 'both'
            ? [...explicitSources.value, ...fracturedSources.value]
            : explicitSources.value.length
              ? explicitSources.value
              : fracturedSources.value

      const value = sumSourcesValue(sources)
    ...
    return {
      ...
      showSourceInfo: computed(() =>
        ...
        props.filter.option == null && (
          hasFilterTag(props.filter, FilterTag.Pseudo) ||
          (
...
//      toggleFractured,
      ...
      toggleFracturedMode,
      fractureDisplayRoll,
      mergeGroup: computed(() => props.filter.mergeGroup),
    }
  }
})
</script>
