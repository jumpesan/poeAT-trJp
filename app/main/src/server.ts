...
export function onEventAnyClientWithSocket<Name extends IpcEvent['name']> (
  name: Name,
  cb: (payload: IpcEventPayload<Name>, client: WebSocket) => void
) {
  evBus.on(name, cb)
}
...
export function sendEventToClient (
  ...
  if (client == null || client.readyState !== 1) return false
...
export interface ServerEvents {
  ...
  onEventAnyClientWithSocket: typeof onEventAnyClientWithSocket
  sendEventTo: typeof sendEventTo
  sendEventToClient: typeof sendEventToClient
}
export const eventPipe = {
  onEventAnyClient,
  onEventAnyClientWithSocket,
  sendEventTo,
  sendEventToClient
}
...
export async function startServer (
  ...
  websocketServer.on('connection', (socket) => {
    ...
    socket.on('message', (bytes) => {
      ...
      evBus.emit(event.name, event.payload, socket)
    })
    socket.on('close', () => {
